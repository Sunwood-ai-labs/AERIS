...
fn save_settings(app: tauri::AppHandle, state: State<SharedState>, settings: Settings) -> Result<(),String> {
    ...
    if let Some(w)=app.get_webview_window("gadget") {
        ...
        let width=if settings.mini {560.} else {340.}; let height=if settings.mini {76.} else {520.};
...
async fn show_gadget(app: tauri::AppHandle, state: State<'_, SharedState>) -> Result<(), String> {
    ...
    let w=WebviewWindowBuilder::new(&app, "gadget", WebviewUrl::App("index.html?view=gadget".into()))
        .title("AERIS — ガジェット").inner_size(if settings.mini {560.} else {340.}, if settings.mini {76.} else {520.})
        ...
        .min_inner_size(if settings.mini {560.} else {340.}, if settings.mini {76.} else {520.})
        .max_inner_size(if settings.mini {560.} else {340.}, if settings.mini {76.} else {520.})
    ...
    if let Ok(Some(m))=w.current_monitor() {
        ...
        let _=w.set_position(tauri::LogicalPosition::new((pos.x as f64 + size.width as f64)/scale-width-28., pos.y as f64/scale+60.));
    }
    Ok(())
}
