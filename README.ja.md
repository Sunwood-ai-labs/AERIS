...
- **3つの表示形態**：詳細なメイン画面、340 × 680のガジェット、560 × 76のミニバー。
...
| デスクトップガジェット | 横長ミニバー |
|---|---|
| <img src="docs/screenshots/gadget.png" alt="340×680の常駐ガジェット" width="340"> | <img src="docs/screenshots/mini.png" alt="560×76のミニバーをブラウザー内に表示" width="560"> |
...
[Releases](https://github.com/Sunwood-ai-labs/AERIS/releases/latest) から、自分のOS・CPUに合うファイルを選んでください。以下の `1.2.4` はバージョン番号です。

| OS / CPU | 配布ファイル | 起動方法 |
|---|---|---|
| Windows x64 | `AERIS-1.2.4-windows-x64-setup.exe` | インストーラーを実行 |
| Windows x64・ポータブル | `AERIS-1.2.4-windows-x64-portable.zip` | フォルダー全体を展開して `AERIS/AERIS.exe` を起動 |
| macOS・Apple Silicon | `AERIS-1.2.4-macos-arm64.dmg` | 開いてAERISをApplicationsへコピー |
| macOS・Intel | `AERIS-1.2.4-macos-x64.dmg` | 開いてAERISをApplicationsへコピー |
| Linux x64・Debian/Ubuntu系 | `AERIS-1.2.4-linux-x64.deb` | `sudo apt install ./AERIS-1.2.4-linux-x64.deb` |
| Linux x64・AppImage | `AERIS-1.2.4-linux-x64.AppImage` | 実行権限を付けて起動 |
...
**アプリ別の積み上げバー：** 概要画面では同じ実行ファイル名のプロセスを合算し、CPUとメモリで同じアプリに同じ色を付けます。上位アプリを色と凡例で表示し、残りは「その他」にまとめます。色の区画や凡例を押すと、そのアプリのプロセス一覧へ移動できます。CPUはバー上部のボタンで時間推移にも切り替え可能。ガジェットはCPUを上、MEMを下に並べ、それぞれの積み上げ内訳と上位3アプリを同時表示します。
...
ガジェット上部のメモリ容量バーもアプリ別の色で表示します。色付き部分の幅は実際の物理メモリ使用率で、右側は空き容量。色付き部分の中を計測したプロセスメモリの比率で配分するため、**各アプリの物理RAM占有量を表すものではありません**。注記とツールチップで基準を確認でき、下部のCPU・MEMの内訳も同時に見られます。
...
3. `v1.2.4` のようなバージョンタグをpushすると全構成をビルドし、**全構成の成功後**にGitHub Releaseを作成・公開します。
