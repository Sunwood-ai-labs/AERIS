...
export type Performance = { gpu?:GpuSnapshot|null; cpus:{name:string;usage:number;frequency:number}[]; totalSwap:number; usedSwap:number; disks:{name:string;mount:string;kind:string;filesystem:string;total:number;available:number;read:number;write:number}[]; interfaces:{name:string;mac:string;download:number;upload:number;totalReceived:number;totalTransmitted:number}[] };
