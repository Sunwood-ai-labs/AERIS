...
export type GpuSnapshot={adapters:{id:string;name:string;usage:number|null;dedicatedUsed:number|null;sharedUsed:number|null;dedicatedTotal:number;sharedTotal:number}[];error:string|null;warming:boolean};
export type Performance = { networkOwners?:{owners:{pid:number;tcp:number;udp:number}[];error:string|null}; gpu?:GpuSnapshot|null; cpus:{name:string;usage:number;frequency:number}[]; totalSwap:number; usedSwap:number; disks:{name:string;mount:string;kind:string;filesystem:string;total:number;available:number;read:number;write:number}[]; interfaces:{name:string;mac:string;download:number;upload:number;totalReceived:number;totalTransmitted:number}[] };
